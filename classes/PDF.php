...
/**
 * PDF Class - ProcessWire PDF Generation Helper
 * 
 * This class provides methods to generate PDF documents using mPDF library.
 * It supports converting HTML to PDF, including files with variables, and creating multi-page PDFs.
 * 
 * Usage Examples:
 * 
 * 1. Simple HTML to PDF:
 *    $pdf = new PDF();
 *    $pdf->html2pdf('<h1>Hello World</h1>', ['output' => 'INLINE']);
 * 
 * 2. Convert file to PDF:
 *    $pdf->file2pdf('/path/to/template.php', ['name' => 'John'], ['output' => 'INLINE']);
 * 
 * 3. Multi-page PDF:
 *    $pages = [
 *        ['page1' => '/path/to/page1.php', 'data' => 'value1'],
 *        ['page2' => '/path/to/page2.php', 'data' => 'value2']
 *    ];
 *    $pdf->generatePDF($pages, [], ['output' => 'INLINE', 'dest' => '/tmp/']);
 * 
 * Available output options: INLINE (browser), DOWNLOAD (force download), FILE (save to disk)
 * Default font: Arial (built-in font, no custom fonts loaded)
 * 
...
class PDF extends WireData {
  ...
  public function options($array = []) {
    ...
    $options = [
      ...
      'margin_header' => !empty($array['margin_header']) ? $array['margin_header'] : 10,
      'margin_footer' => !empty($array['margin_footer']) ? $array['margin_footer'] : 10,
      'output' => !empty($array['output']) ? $array['output'] : "INLINE",
      'dest' => !empty($array['dest']) ? $array['dest'] : "",
  ...
  public function html2pdf($html, $params = []) {
    ...
    // mPDF
    require_once($this->nautilus->path() . "lib/mpdf/vendor/autoload.php");
    ...
    $stylesheet = file_get_contents($this->nautilus->path() . 'lib/css/mpdf.css');
